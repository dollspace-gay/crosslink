...
- [ ] AC-6: Migration system uses `PRAGMA user_version` correctly; new test `test_migration_idempotent` runs all migrations twice on the same database and asserts schema is identical both times (validates REQ-6)
...
Replace the current migration system (`db.rs:137-155`) which silently ignores all "duplicate column"/"already exists" errors. New approach:

- Read `PRAGMA user_version` directly (not via `query_row` on a non-existent column)
- Execute migrations gated on version: `if current_version < N { run_migration_N(); set user_version = N; }`
- Each migration runs in its own transaction with rollback on failure
- New test: apply all 15 migrations to fresh DB, assert schema matches; apply all 15 again, assert no errors and no schema changes.
