...
/kickoff "add batch retry logic" --verify thorough
```
:::
...
3. Documents its plan via `crosslink comment`
...
::: {.callout-note collapse="true" title="Manual CLI equivalent"}
```bash
# List all active kickoff agents
crosslink kickoff list

# JSON output for scripting
crosslink --json kickoff list

# Filter by status
crosslink kickoff list --status running
crosslink kickoff list --status done
```

Discovery checks three sources:

- **Worktrees** -- scans `.worktrees/` for active agent directories
- **tmux** -- queries tmux for sessions matching the `feat-*` prefix
- **Docker** -- queries Docker/Podman for crosslink agent containers
:::
...
::: {.callout-note collapse="true" title="Manual CLI equivalent"}
```bash
# Automated cleanup of all finished agents
crosslink kickoff cleanup
