...
fn main() {
    ...
    if rules_dir.is_dir() {
        ...
        let mut gen_file = fs::File::create(&gen_path).expect("Failed to create rules_gen.rs");
        ...
        .unwrap();
        writeln!(gen_file, "// Generated from resources/crosslink/rules/").unwrap();
        ...
        .unwrap();
        for (filename, const_name) in &rule_entries {
            writeln!(gen_file, "    (\"{}\", {}),", filename, const_name).unwrap();
        }
        writeln!(gen_file, "];").unwrap();
    }
}
