...
fn test_parallel_lock_claim_one_winner() {
    ...
    // The loser must produce an informative message, not silent garbage.
    if !success_a {
        assert!(
            stderr_a.contains("lock")
                || stderr_a.contains("Lock")
                || stderr_a.contains("locked")
                || stderr_a.contains("claimed")
                || stderr_a.contains("conflict")
                || stdout_a.contains("lock")
                || stdout_a.contains("locked"),
            "Losing agent-a should report a lock-related message.\nstdout: {}\nstderr: {}",
            stdout_a,
            stderr_a,
        );
    }
    if !success_b {
        assert!(
            stderr_b.contains("lock")
                || stderr_b.contains("Lock")
                || stderr_b.contains("locked")
                || stderr_b.contains("claimed")
                || stderr_b.contains("conflict")
                || stdout_b.contains("lock")
                || stdout_b.contains("locked"),
            "Losing agent-b should report a lock-related message.\nstdout: {}\nstderr: {}",
            stdout_b,
            stderr_b,
        );
    }
...
fn test_offline_local_operations_then_sync() {
    ...
    assert!(
        stdout.contains("Offline issue beta"),
        "list should show beta\nstdout: {}",
        stdout
    );

    // --- Offline show ---
    let (ok, stdout, stderr) = run(&["issue", "show", "1"]);
    assert!(
        ok,
    ...
    // --- Offline update ---
    let (ok, stdout, stderr) = run(&["issue", "update", "2", "-t", "Offline issue beta (updated)"]);
    assert!(
        ok,
    ...
    let (ok, stdout, _) = run(&["issue", "show", "2"]);
