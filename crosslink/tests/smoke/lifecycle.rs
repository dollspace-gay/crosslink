...
/// `swarm list` should exit cleanly or indicate no hub cache is initialized.
#[test]
fn test_swarm_list_empty() {
    let h = SmokeHarness::new();

    let result = h.run(&["swarm", "list"]);
    let combined = format!("{}{}", result.stdout, result.stderr);
    // Acceptable outcomes: success with empty list, or a clear error about
    // missing hub cache (requires `crosslink sync` first).
    assert!(
        result.success
            || combined.contains("No")
            || combined.contains("no")
        ...
        "swarm list should handle gracefully.\nstdout: {}\nstderr: {}",
        result.stdout,
        result.stderr,
    );
}
