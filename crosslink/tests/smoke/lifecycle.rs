...
fn extract_issue_id(stdout: &str) -> String {
    for line in stdout.lines() {
        // "Created issue L1" (offline local ID)
    ...
    // Fallback: look for "Now working on: L1" style
    for line in stdout.lines() {
        if line.contains("working on") {
            let parts: Vec<&str> = line.split_whitespace().collect();
            for part in &parts {
                if part.starts_with('L') && part[1..].chars().all(|c| c.is_ascii_digit()) {
...
/// `swarm list` should exit cleanly even with no active swarms.
#[test]
fn test_swarm_list_empty() {
    let h = SmokeHarness::new();

    let result = h.run_ok(&["swarm", "list"]);
