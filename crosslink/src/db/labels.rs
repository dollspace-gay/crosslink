...
impl Database {
    ...
    pub fn get_labels_batch(&self, issue_ids: &[i64]) -> Result<std::collections::HashMap<i64, Vec<String>>> {
        use std::collections::HashMap;

        let mut result: HashMap<i64, Vec<String>> = issue_ids.iter().map(|&id| (id, Vec::new())).collect();
        ...
        let rows = stmt.query_map(
            rusqlite::params_from_iter(issue_ids.iter()),
            |row| {
