...
fn calculate_progress(db: &Database, issue: &Issue) -> Result<Option<SubissueProgress>> {
    ...
    let completed = subissues.iter().filter(|s| s.status == crate::models::IssueStatus::Closed).count() as i32;
...
pub fn run(db: &Database, crosslink_dir: &std::path::Path) -> Result<()> {
    ...
    for issue in &all_ready {
        ...
        scored.push(ScoredIssue { issue: issue.clone(), score, progress });
    }
    ...
    if let Some(ref p) = top.progress {
        println!("       Progress: {}/{} subissues complete", p.completed, p.total);
    }
