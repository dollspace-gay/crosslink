...
### Added
- Poll updates only for active tab in TUI main event loop (#592)
- Derive tab count dynamically in TUI tab command error message (#591)
- Rename dominated variable to excluded_by_filter in issue tree (#589)
- Optimize knowledge_tab apply_filters to avoid full page clone (#588)
- Invoke original panic hook in TUI TerminalGuard panic handler (#587)
- Rename ViewMode enums to be tab-specific for grep-ability (#585)
- Remove unused _edit_text field from ConfigTab (#583)
- Extract shared make_key test helper from duplicated TUI test modules (#575)
- Extract shared StatusFilter enum from issues_tab and milestones_tab (#574)
- Consolidate duplicate truncate_str functions across TUI tabs (#573)
- Consolidate format_relative_time into single shared TUI function (#572)
- Extract shared HIGHLIGHT_BG constant from duplicated TUI tab files (#571)
- Replace raw HashMap with typed query struct for session parameters (#569)
- Optimize knowledge search handler to build title map lazily (#567)
- Implement Serialize directly on WsEvent to eliminate manual dispatch (#566)
- Document config field name mapping between file format and API (#565)
- Derive WsEvent event_type from type instead of manual string field (#563)
- Define enums for priority and comment kind in API request types (#562)
- Move scattered response types into centralized server types module (#560)
- Remove stale module-level allow dead_code from server types (#557)
- Move usage summary aggregation from API client to store layer (#630)
- Cache platform info in VS Code ensureExecutable instead of re-detecting (#627)
- Extract duplicated priorityVariant into shared dashboard utility (#626)
- Rename NavItem component to avoid shadowing NavItem interface (#625)
- Replace browser prompt with dialog for theme import in Appearance (#624)
- Wrap useEffect callbacks properly in dashboard Milestones Knowledge Sync (#623)
- Move eventCounter into Zustand store in orchestrator.ts (#621)
- Rename WsClient closed field to isClosed for clarity (#620)
- Use type predicate instead of cast in dashboard API client issues list (#619)
- Remove redundant response type aliases from dashboard types (#618)
- Remove unused request type interfaces from dashboard types (#617)
- Replace bare except-pass blocks in hook files with specific exception types (#632)
- Extract per-language linters from run_linter god function (#601)
- Extract per-language parsers from get_dependencies god function (#600)
- Replace 9-parameter build_reminder with ReminderContext dataclass (#599)
- Replace 5-tuple return with RulesBundle dataclass in load_all_rules (#616)
- Remove glob-style entry from SKIP_DIRS set in prompt-guard (#615)
- Import shared functions in session-start.py instead of reimplementing (#604)
- Import shared functions in heartbeat.py instead of reimplementing (#603)
- Import find_crosslink_dir from shared module in pre-web-check (#598)
- Replace module-level mutable global with lru_cache for crosslink binary (#614)
...
### Fixed
- Fix config_tab save_array_items always writing to team scope (#590)
- Fix config_tab scroll and cursor desynchronization (#586)
- Add scroll bounds enforcement to milestones and agents detail views (#582)
- Fix unchecked u16 cast for detail scroll in agents_tab (#581)
- Fix load_detail silently failing with no feedback in agents_tab (#579)
- Fix knowledge_tab refresh blocking main thread on git fetch (#578)
- Fix TUI clipboard on Linux to support Wayland and WSL2 environments (#576)
- Fix double database query on every refresh in issues_tab (#570)
- Require trigger_type when comment kind is intervention in API (#568)
- Fix watcher loop spinning on closed notification channel (#561)
- Propagate errors instead of unwrap_or_default in issue detail hydration (#559)
- Fix overly permissive fuzzy agent-worktree matching in server (#558)
- Fix push_hub_cache reporting success when remote is unreachable (#556)
- Fix KnowledgeDetail using create endpoint for page edits (#631)
- Fix suppressed exhaustive-deps lint in Issues.tsx useEffect (#622)
- Fix Dashboard stat cards displaying hardcoded placeholder values (#610)
- Fix Sessions clearWork using magic sentinel value zero (#609)
- Fix naive Cargo.toml parser missing dev and build dependencies (#602)
